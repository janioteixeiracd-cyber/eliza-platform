...
export default function PlatformAdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      {supportMode.active && (
        <div className="fixed top-0 left-0 right-0 z-[100] bg-amber-500 text-white px-8 py-2 flex items-center justify-between shadow-2xl animate-in slide-in-from-top duration-500">
           <div className="flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-[9px] font-bold opacity-80 uppercase mt-0.5">Visualizando Clínica: <span className="underline">{supportMode.clinicData?.name}</span></p>
              </div>
           </div>
           <button 
             onClick={exitSupportMode}
             className="flex items-center gap-2 px-4 py-1.5 bg-white text-amber-600 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-amber-50 transition-colors"
           >
              Encerrar Sessão <X className="w-3 h-3" />
           </button>
        </div>
      )}

      {/* Admin Sidebar */}
      <aside className={`w-72 bg-slate-900 text-white flex flex-col shrink-0 border-r border-white/5 transition-all ${supportMode.active ? 'pt-12' : ''}`}>
        <div className="h-24 flex items-center px-10 gap-4 border-b border-white/5">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-900 font-black text-xl shadow-xl shadow-white/10">E</div>
          <div>
            <h1 className="text-xl font-black tracking-tighter uppercase">ELIZA</h1>
            <p className="text-[9px] text-teal-400 font-black uppercase tracking-widest">Global Admin</p>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-white/5 space-y-6">
          <div className="flex items-center gap-4 px-2">
            <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center font-black text-teal-400 uppercase text-xs border border-white/10 shadow-inner">
              {platformRole?.[0] || 'A'}
            </div>
          ...
          <div className="space-y-2">
            <button 
              onClick={() => navigate('/')}
              className="w-full flex items-center justify-between px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-white/10 transition-all group"
            >
            ...
            <button 
               ...
               className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-rose-400 hover:bg-rose-500/10 transition-all"
            >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className={`h-24 bg-white border-b border-slate-200 flex items-center justify-between px-10 shrink-0 transition-all ${supportMode.active ? 'mt-12' : ''}`}>
          <div className="flex items-center gap-4">
             <div className={`w-1.5 h-1.5 rounded-full ${health === null ? 'bg-slate-300' : health.ok ? 'bg-emerald-500 animate-ping' : 'bg-rose-500 animate-ping'}`}></div>
             <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
               {health === null ? 'Verificando infraestrutura...' : health.ok ? 'Backend e Firestore respondendo' : 'Backend indisponível'}
             </span>
          </div>
          ...
          <AnimatePresence mode="wait">
             {supportMode.active ? (
               <motion.div 
                 ...
                 className="p-8"
               >
                 <div className="bg-white rounded-[3rem] p-12 border border-slate-200 shadow-xl max-w-4xl mx-auto text-center space-y-8">
                    <div className="w-24 h-24 bg-amber-50 text-amber-500 rounded-[2.5rem] flex items-center justify-center mx-auto border-4 border-white shadow-lg">
                       <Eye className="w-10 h-10" />
                    </div>
                    <div>
                       <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Modo Suporte Habilitado</h2>
                       <p className="text-sm text-slate-500 mt-2 font-medium">Você está visualizando os dados restritos da clínica <span className="font-bold text-slate-900 underline">{supportMode.clinicData?.name}</span>.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                       <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left">
                          ...
                          <p className="text-[11px] font-mono font-bold text-slate-700">{supportMode.clinicId}</p>
                       </div>
                       <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left">
                    ...
                    <div className="bg-rose-50 p-6 rounded-3xl border border-rose-100 flex items-start gap-4 text-left">
                    ...
                    <div className="flex gap-4 justify-center">
                       <button
                         ...
                         className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center gap-3 shadow-2xl shadow-slate-900/20"
                       >
